...
public class LHMAuto<K, V> {
	...
	// getSpecificAuto - Return an Automobile that was specified by the client
	public Automobile getSpecificAuto(int n)
	{
		Automobile auto  = null;
		int count = 0;
		Set<K> keys = lHashMap.keySet();
		
		for (K key : keys)
		{
			count++;
			auto = (Automobile) lHashMap.get(key);
			if (count == n)
			{
				break;
			}
